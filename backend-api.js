...
/**
 * 🚀 BigShopper Production Scraper
 * Complete Backend API with Parallel Scraping
 * 
 * Features:
 * - Parallel scraping (configurable workers)
  ...
  try {
    ...
    await supabase
      ...
      .update({
        ...
        total_scrapes: (await supabase.from('scrapes').select('id').eq('ean_id', eanId)).data.length,
...
// Get EAN data with history
app.get('/api/ean/:barcode', async (req, res) => {
  try {
...
async function start() {
  try {
    ...
    app.listen(PORT, () => {
      console.log(`\n${'='.repeat(60)}`);
      console.log(`🚀 BigShopper Scraper API running on port ${PORT}`);
      console.log('='.repeat(60));
      console.log(`Environment: ${NODE_ENV}`);
      console.log(`Parallel workers: ${PARALLEL_WORKERS}`);
      console.log(`Queue: ${REDIS_URL}\n`);
    });
